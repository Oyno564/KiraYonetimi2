...
const API = "http://localhost:5036/api";
...
function AuthProvider({ children }) {
  ...
  const logout = () => {
    ...
    localStorage.removeItem("kira_token"); localStorage.removeItem("kira_user");
  };
      ...
      <div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, display: "flex", flexDirection: "column", gap: 8 }}>
        {toasts.map(t => (
          <div key={t.id} style={{
            ...
            color: "#fff", boxShadow: "0 4px 20px rgba(0,0,0,0.2)", animation: "slideIn 0.3s ease"
          }}>{t.msg}</div>
        ))}
      </div>
    </ToastCtx.Provider>
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "radial-gradient(ellipse at 50% 0%, rgba(240,165,0,0.08) 0%, var(--bg) 70%)" }}>
      <div style={{ width: 420, animation: "fadeUp 0.4s ease" }}>
        <div style={{ textAlign: "center", marginBottom: 40 }}>
          ...
          <p style={{ color: "var(--muted)", marginTop: 8 }}>Yönetim paneline giriş yapın</p>
        </div>
        <Card>
          <Field label="E-posta">
...
function ApartsPage() {
  ...
  const emptyForm = {
    apartBlock: "1", apartFloor: "", apartNo: "",
    apartStatus: "false", apartOwnerOrTenant: "false",
    apartTypePkId: "", userPkId: ""
  };
  const [form, setForm] = useState(emptyForm);
  const [editForm, setEditForm] = useState({});

  const FLOORS = [0,1,2,3,4,5];
  const NOS    = Array.from({ length: 15 }, (_, i) => i + 1);
  const floorLabel = (n) => { const num = parseInt(n); return num === 0 ? "Zemin Kat" : `${num}. Kat`; };

  const load = () =>
    authFetch("/Apart/GetAllAparts")
      .then(r => r.json())
  ...
  useEffect(() => {
    load();
    authFetch("/ApartType").then(r => r.json()).then(d => setApartTypes(Array.isArray(d) ? d : [])).catch(() => {});
    authFetch("/User").then(r => r.json()).then(d => setUsers(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);
  ...
  const openEdit = (a) => {
    setEditTarget(a);
    setEditForm({
      apartStatus: a.apartStatus ? "true" : "false",
      apartOwnerOrTenant: a.apartOwnerOrTenant ? "true" : "false",
      apartTypePkId: a.apartTypePkId || "",
      userPkId: a.userPkId || "",
    });
    setEditModal(true);
  };

  const deleteApart = async (a) => {
    try {
      const res = await authFetch(`/Apart/Delete/${a.pkId}`, { method: "DELETE" });
  ...
  const save = async () => {
    if (form.apartFloor === "") { toast.error("Kat seçin"); return; }
    if (!form.apartNo)          { toast.error("Daire no seçin"); return; }
    if (!form.apartTypePkId)    { toast.error("Daire tipi seçin"); return; }
    try {
      const payload = {
        apartBlock:         1,
        apartFloor:         parseInt(form.apartFloor),
        apartNo:            parseInt(form.apartNo),
        apartStatus:        form.apartStatus === "true",
        apartOwnerOrTenant: form.apartOwnerOrTenant === "true",
        apartTypePkId:      form.apartTypePkId,
        userPkId:           form.userPkId || null,
      };
      const res = await authFetch("/Apart/Create", { method: "POST", body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
  ...
  const saveEdit = async () => {
    try {
      const payload = {
        apartBlock:         editTarget.apartBlock,
        apartFloor:         editTarget.apartFloor,
        apartNo:            editTarget.apartNo,
        apartStatus:        editForm.apartStatus === "true",
        apartOwnerOrTenant: editForm.apartOwnerOrTenant === "true",
        apartTypePkId:      editForm.apartTypePkId || editTarget.apartTypePkId,
        userPkId:           editForm.userPkId || null,
      };
      const res = await authFetch(`/Apart/Update/${editTarget.pkId}`, { method: "PUT", body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
  ...
  const getTypeName = (apartTypeId) => {
    const t = apartTypes.find(t => t.apartTypeId === apartTypeId);
    return t ? t.typeName : (apartTypeId || "-");
  };

  const iconBtn = (onClick, icon, title, color = "var(--muted)") => (
    <button onClick={onClick} title={title} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color, padding: "4px 6px" }}>{icon}</button>
  );
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 32 }}>
        ...
        <Btn onClick={openModal}>+ Yeni Daire</Btn>
      </div>

      {loading ? <Loader /> : data.length === 0 ? <EmptyState icon="🏠" msg="Henüz daire eklenmemiş" /> : (
        <Card style={{ padding: 0 }}>
          <table>
            <thead><tr><th>Blok</th><th>Kat</th><th>No</th><th>Tip</th><th>Durum</th><th>Mülkiyet</th><th></th></tr></thead>
            <tbody>
              {data.map((a, i) => (
                <tr key={a.pkId || i}>
                  <td style={{ fontWeight: 600 }}>Blok {a.apartBlock}</td>
                  <td>{floorLabel(a.apartFloor)}</td>
                  <td>No {a.apartNo}</td>
                  <td>{getTypeName(a.apartTypeId)}</td>
                  ...
                  <td style={{ whiteSpace: "nowrap" }}>
                    {iconBtn(() => openEdit(a), "✏️", "Düzenle")}
                    {iconBtn(() => setConfirmDelete(a), "🗑️", "Sil", "#ef4444")}
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Yeni Daire Ekle">
        <Field label="Blok">
          <select disabled style={{ opacity: 0.6 }}><option>Blok 1</option></select>
        </Field>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="Kat">
            <select value={form.apartFloor} onChange={e => setForm(f => ({ ...f, apartFloor: e.target.value }))}>
              <option value="">Seçin</option>
              {FLOORS.map(n => <option key={n} value={n}>{floorLabel(n)}</option>)}
            </select>
          </Field>
          <Field label="Daire No">
            <select value={form.apartNo} onChange={e => setForm(f => ({ ...f, apartNo: e.target.value }))}>
              <option value="">Seçin</option>
              {NOS.map(n => <option key={n} value={n}>No {n}</option>)}
            </select>
          </Field>
        </div>
        <Field label="Daire Tipi">
          <select value={form.apartTypePkId} onChange={e => setForm(f => ({ ...f, apartTypePkId: e.target.value }))}>
            <option value="">{apartTypes.length === 0 ? "— Daire tipi yok —" : "Seçin"}</option>
            {apartTypes.map(t => <option key={t.apartTypePkId} value={t.apartTypePkId}>{t.typeName}</option>)}
          </select>
        </Field>
        <Field label="Kiracı (opsiyonel)">
          <select value={form.userPkId} onChange={e => setForm(f => ({ ...f, userPkId: e.target.value }))}>
            <option value="">{users.length === 0 ? "— Kullanıcı yok —" : "Boş bırak"}</option>
            {users.map(u => <option key={u.pkId} value={u.pkId}>{u.fullName}{u.phone ? ` (${u.phone})` : ""}</option>)}
          </select>
        </Field>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="Doluluk">
            <select value={form.apartStatus} onChange={e => setForm(f => ({ ...f, apartStatus: e.target.value }))}>
              <option value="false">Boş</option>
              <option value="true">Dolu</option>
            </select>
          </Field>
          <Field label="Mülkiyet">
            <select value={form.apartOwnerOrTenant} onChange={e => setForm(f => ({ ...f, apartOwnerOrTenant: e.target.value }))}>
      ...
      {/* DÜZENLE MODAL */}
      <Modal open={editModal} onClose={() => setEditModal(false)} title={`Daire Düzenle — Kat ${editTarget?.apartFloor} / No ${editTarget?.apartNo}`}>
        <Field label="Daire Tipi">
          <select value={editForm.apartTypePkId} onChange={e => setEditForm(f => ({ ...f, apartTypePkId: e.target.value }))}>
            <option value="">Mevcut tip</option>
            {apartTypes.map(t => <option key={t.apartTypePkId} value={t.apartTypePkId}>{t.typeName}</option>)}
          </select>
        </Field>
        <Field label="Kiracı">
          <select value={editForm.userPkId} onChange={e => setEditForm(f => ({ ...f, userPkId: e.target.value }))}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="Doluluk">
            <select value={editForm.apartStatus} onChange={e => setEditForm(f => ({ ...f, apartStatus: e.target.value }))}>
              <option value="false">Boş</option>
              <option value="true">Dolu</option>
            </select>
          </Field>
          <Field label="Mülkiyet">
            <select value={editForm.apartOwnerOrTenant} onChange={e => setEditForm(f => ({ ...f, apartOwnerOrTenant: e.target.value }))}>
              <option value="false">Kiracı</option>
              <option value="true">Mal Sahibi</option>
            </select>
          </Field>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 8 }}>
          <Btn variant="ghost" onClick={() => setEditModal(false)}>İptal</Btn>
          <Btn onClick={saveEdit}>Güncelle</Btn>
        </div>
      </Modal>

      {/* SİLME ONAY MODAL */}
      <Modal open={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Daire Sil" width={400}>
        <p style={{ marginBottom: 20, color: "var(--muted)" }}>
          <strong style={{ color: "var(--text)" }}>Kat {confirmDelete?.apartFloor} / No {confirmDelete?.apartNo}</strong> dairesini silmek istediğinize emin misiniz?
          <br /><span style={{ fontSize: 13, marginTop: 8, display: "block", color: "#ef4444" }}>Bu dairedeki kiracı da boşa çıkacak.</span>
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <Btn variant="ghost" onClick={() => setConfirmDelete(null)}>İptal</Btn>
          <Btn variant="danger" onClick={() => deleteApart(confirmDelete)}>Evet, Sil</Btn>
        </div>
      </Modal>
    </div>
  );
...
function UsersPage() {
  ...
  const load = () =>
    authFetch("/User").then(r => r.json()).then(d => setData(Array.isArray(d) ? d : [])).catch(() => setData([])).finally(() => setLoading(false));

  useEffect(() => { load(); }, []);

  const deleteUser = async (u) => {
    try {
      const res = await authFetch(`/User/${u.pkId}`, { method: "DELETE" });
      if (!res.ok) throw new Error();
  ...
  return (
    <div>
      <div style={{ marginBottom: 32 }}>
        <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, marginBottom: 4 }}>Kiracılar</h2>
        <p style={{ color: "var(--muted)" }}>{data.length} kişi kayıtlı</p>
      </div>

      {loading ? <Loader /> : data.length === 0 ? <EmptyState icon="👥" msg="Henüz kiracı yok" /> : (
        <Card style={{ padding: 0 }}>
          <table>
            <thead>
              <tr><th>Ad Soyad</th><th>E-posta</th><th>Telefon</th><th>Plaka</th><th>Rol</th><th></th></tr>
            </thead>
            <tbody>
              {data.map((u, i) => (
                <tr key={u.pkId || i}>
                  ...
                  <td>
                    <button onClick={() => setConfirmDelete(u)} title="Sil" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "#ef4444", padding: "4px 6px" }}>🗑️</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal open={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Kiracı Sil" width={400}>
        <p style={{ marginBottom: 20, color: "var(--muted)" }}>
          <strong style={{ color: "var(--text)" }}>{confirmDelete?.fullName}</strong> adlı kiracıyı silmek istediğinize emin misiniz?
          <br /><span style={{ fontSize: 13, marginTop: 8, display: "block", color: "#ef4444" }}>Kiracı bir dairedeyse o daire de boşa çıkacak.</span>
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <Btn variant="ghost" onClick={() => setConfirmDelete(null)}>İptal</Btn>
          <Btn variant="danger" onClick={() => deleteUser(confirmDelete)}>Evet, Sil</Btn>
        </div>
      </Modal>
    </div>
  );
...
function InvoicesPage() {
  ...
  const [editTarget, setEditTarget] = useState(null);
  const [editForm, setEditForm] = useState({});
  const [form, setForm] = useState({ invoiceMonth: "", invoiceYear: new Date().getFullYear(), invoiceAmount: "", invoiceStatus: "false", apartId: "" });
  ...
  const load = async () => {
    try {
      let res = await authFetch("/Invoice");
      if (!res.ok) res = await authFetch("/Invoice/GetAllInvoices");
  ...
  useEffect(() => {
    load();
    authFetch("/Apart/GetAllAparts").then(r => r.json()).then(d => setAparts(Array.isArray(d) ? d : [])).catch(() => {});
  }, []);

  const getApartLabel = (apartId) => {
    const a = aparts.find(a => a.apartId === apartId);
    return a ? `Blok ${a.apartBlock} / Kat ${a.apartFloor} / No ${a.apartNo}` : (apartId || "-");
  };

  const save = async () => {
    if (!form.invoiceMonth) { toast.error("Ay seçin"); return; }
    if (!form.invoiceAmount) { toast.error("Tutar girin"); return; }
    if (!form.apartId) { toast.error("Daire seçin"); return; }
    try {
      const payload = {
        invoiceMonth:  parseInt(form.invoiceMonth),
        invoiceYear:   parseInt(form.invoiceYear),
        invoiceAmount: parseFloat(form.invoiceAmount),
        invoiceStatus: form.invoiceStatus === "true",
        apartId:       parseInt(form.apartId),
      };
      const res = await authFetch("/Invoice", { method: "POST", body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
      toast.success("Fatura oluşturuldu");
      setModal(false);
      setForm({ invoiceMonth: "", invoiceYear: new Date().getFullYear(), invoiceAmount: "", invoiceStatus: "false", apartId: "" });
      load();
    } catch { toast.error("Hata oluştu"); }
  };
  ...
  const openEdit = (inv) => {
    setEditTarget(inv);
    setEditForm({
      invoiceMonth:  String(inv.invoiceMonth),
      invoiceYear:   String(inv.invoiceYear),
      invoiceAmount: String(inv.invoiceAmount),
      invoiceStatus: inv.invoiceStatus ? "true" : "false",
      apartId:       String(inv.apartId),
    });
    setEditModal(true);
  };
  ...
  const saveEdit = async () => {
    try {
      const payload = {
        invoiceMonth:  parseInt(editForm.invoiceMonth),
        invoiceYear:   parseInt(editForm.invoiceYear),
        invoiceAmount: parseFloat(editForm.invoiceAmount),
        invoiceStatus: editForm.invoiceStatus === "true",
        apartId:       parseInt(editForm.apartId),
      };
      const res = await authFetch(`/Invoice/${editTarget.pkId}`, { method: "PUT", body: JSON.stringify(payload) });
      if (!res.ok) throw new Error();
  ...
  return (
    <div>
      ...
      {loading ? <Loader /> : data.length === 0 ? <EmptyState icon="📄" msg="Henüz fatura oluşturulmamış" /> : (
        <Card style={{ padding: 0 }}>
          <table>
            <thead><tr><th>Dönem</th><th>Tutar</th><th>Durum</th><th>Daire</th><th></th></tr></thead>
            <tbody>
              {data.map((inv, i) => (
                <tr key={inv.pkId || i}>
                  ...
                  <td style={{ color: "var(--muted)", fontSize: 13 }}>{getApartLabel(inv.apartId)}</td>
                  <td>
                    <button onClick={() => openEdit(inv)} title="Düzenle" style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: "var(--muted)", padding: "4px 6px" }}>✏️</button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Yeni Fatura Ekle">
        <Field label="Daire">
          <select value={form.apartId} onChange={e => setForm(f => ({ ...f, apartId: e.target.value }))}>
            <option value="">Daire seçin</option>
            {aparts.map(a => (
              <option key={a.apartId} value={a.apartId}>
                Blok {a.apartBlock} / Kat {a.apartFloor} / No {a.apartNo}
              </option>
            ))}
          </select>
        </Field>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <Field label="Yıl">
            <select value={form.invoiceYear} onChange={e => setForm(f => ({ ...f, invoiceYear: e.target.value }))}>
              {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
          </Field>
        </div>
        <Field label="Tutar (₺)">
          <input type="number" value={form.invoiceAmount} onChange={e => setForm(f => ({ ...f, invoiceAmount: e.target.value }))} placeholder="3500" />
        </Field>
      ...
      {/* FATURA GÜNCELLE MODAL */}
      <Modal open={editModal} onClose={() => setEditModal(false)} title="Fatura Güncelle">
        <Field label="Daire">
          <select value={editForm.apartId} onChange={e => setEditForm(f => ({ ...f, apartId: e.target.value }))}>
            <option value="">Daire seçin</option>
            {aparts.map(a => (
              <option key={a.apartId} value={a.apartId}>
                Blok {a.apartBlock} / Kat {a.apartFloor} / No {a.apartNo}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <Field label="Ay">
            <select value={editForm.invoiceMonth} onChange={e => setEditForm(f => ({ ...f, invoiceMonth: e.target.value }))}>
              {MONTHS.map((m, i) => <option key={i} value={i + 1}>{m}</option>)}
            </select>
          </Field>
          <Field label="Yıl">
            <select value={editForm.invoiceYear} onChange={e => setEditForm(f => ({ ...f, invoiceYear: e.target.value }))}>
              {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
            </select>
          </Field>
        </div>
        <Field label="Tutar (₺)">
          <input type="number" value={editForm.invoiceAmount} onChange={e => setEditForm(f => ({ ...f, invoiceAmount: e.target.value }))} />
        </Field>
        <Field label="Ödeme Durumu">
          <select value={editForm.invoiceStatus} onChange={e => setEditForm(f => ({ ...f, invoiceStatus: e.target.value }))}>
            <option value="false">Bekliyor</option>
            <option value="true">Ödendi</option>
          </select>
        </Field>
...
function MessagesPage() {
  ...
  const send = async () => {
    if (!form.toUserId) { toast.error("Alıcı seçin"); return; }
    if (!form.messageContent) { toast.error("Mesaj yazın"); return; }
    try {
  ...
  const getUserName = (id) => {
    ...
    return u ? u.fullName : `Kullanıcı #${id}`;
  };
            ...
            <Card key={m.pkId || i} style={{ animation: "fadeUp 0.3s ease" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{getUserName(m.fromUserId)} → {getUserName(m.toUserId)}</div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title="Yeni Mesaj Gönder">
        <Field label="Alıcı">
          <select value={form.toUserId} onChange={e => setForm(f => ({ ...f, toUserId: e.target.value }))}>
            ...
            {users.map(u => <option key={u.pkId} value={u.userId}>{u.fullName} {u.phone ? `(${u.phone})` : ""}</option>)}
          </select>
        </Field>
        <Field label="Mesaj">
